...
export const getCompanyMenuItems = (t: any, permissions: any[], auth: any,referralSettings: any): NavItem[] => {
    ...
    // Business Directory
    if (false && hasPermission(permissions, 'manage-business-directory')) {
        items.push({
    ...
    if (false && vCardChildren.length > 0) {
        items.push({
    ...
    if (false && nfcCardChildren.length > 0) {
        items.push({
            title: t('NFC Cards'),
            icon: Nfc,
    ...
    // Campaigns
    if (false && hasPermission(permissions, 'manage-campaigns')) {
        items.push({
            title: t('Campaigns'),
