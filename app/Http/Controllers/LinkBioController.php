...
class LinkBioController extends Controller
{
    ...
    public function create()
    {
        ...
        $isCompanyOrSuperAdmin = false;
        if ($user->type === 'superadmin' || $user->type === 'super admin' || $user->type === 'company') {
            $isCompanyOrSuperAdmin = true;
        } elseif ($user->created_by) {
            $parent = User::find($user->created_by);
            if ($parent && ($parent->type === 'superadmin' || $parent->type === 'super admin' || $parent->type === 'company')) {
                $isCompanyOrSuperAdmin = true;
            }
        }

        // Check plan limits only for non-admin/non-company users
        if (!$isCompanyOrSuperAdmin) {
        ...
        $userRole = $isCompanyOrSuperAdmin ? 'superadmin' : ($user->roles->pluck('name')->first() ?? $user->type);
    ...
    public function store(Request $request)
    {
        ...
        $isCompanyOrSuperAdmin = false;
        if ($authUser->type === 'superadmin' || $authUser->type === 'super admin' || $authUser->type === 'company') {
            $isCompanyOrSuperAdmin = true;
        } elseif ($authUser->created_by) {
            $parent = User::find($authUser->created_by);
            if ($parent && ($parent->type === 'superadmin' || $parent->type === 'super admin' || $parent->type === 'company')) {
                $isCompanyOrSuperAdmin = true;
            }
        }
